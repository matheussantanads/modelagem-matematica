...
def modelo_verhulst(p, t, P_infinito, r):
    dpdt = r*(1-(p/P_infinito))*p
    return dpdt

def residual(p):
    p = tuple(p)
    sim_P = odeint(modelo_verhulst, p0, dados_anos_censo, args=p).flatten()
...
estimativa = odeint(modelo_verhulst, p0, dados_anos_censo, args=(pInfinito_Ajustado, r_Ajustado)) #Resolve o modelo com os parâmetros ajustados
